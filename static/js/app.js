...
function bindEvents() {
  ...
  elements.sidebarTree.addEventListener("click", (event) => {
    ...
    if (!treeRow) return;
    scrollToFolder(treeRow.dataset.treePath);
  });
...
function renderBoard() {
  ...
  elements.main.innerHTML = groupedScripts.map(([groupName, scripts]) => `
    <div class="script-group" data-group-folder="${escapeAttr(groupName)}">
...
function renderTree(node, parentPath, depth) {
  return Object.keys(node.children)
    ...
    .map((name) => {
      ...
      const isActive = appState.activePath === fullPath;
...
// 点击目录树：保持完整列表，平滑滚动到对应目录的分组（不筛选）。
// 父级节点定位到其第一个子目录分组；目标不在当前视图时退而求其次，
// 滚动到排序在其后的下一个分组（与列表的 folder 排序一致）。
function scrollToFolder(treePath) {
  appState.activePath = treePath;
  if (appState.activeMode !== "all") {
    appState.activeMode = "all";
    render();
  } else {
    renderSidebar();
  ...
  const groups = [...elements.main.querySelectorAll("[data-group-folder]")];
  const target =
    groups.find((el) => el.dataset.groupFolder === treePath) ||
    groups.find((el) => el.dataset.groupFolder.startsWith(`${treePath}/`)) ||
    groups.find((el) => el.dataset.groupFolder.toLowerCase() > lowerPath);
